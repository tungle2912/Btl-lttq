...
namespace HotelManager.pages
{
    public partial class Reservation : Form
    {
		...
		private void search()
		{
            string name = BillSearchTextBox.Text.Trim();
		    ...
		    if (name != "")
			   {
				try
				{
                    dt = db.GetData($"select * from billview(N'{name}','{time}')");
					reservationDataGridView.DataSource = dt;
				}
				catch (Exception ex)
				{
			...
			else
			{
				try
				{
						dt = db.GetData($"select * from billview(null,'{time}')");
						reservationDataGridView.DataSource = dt;
				}
				catch (Exception ex)
				{
            ...
            else   //khong tim kiem theo ngay
            {
				if (name != "")
				{                 
                    try
                    {
						dt = db.GetData($"select * from billview(N'{name}',null)");
						reservationDataGridView.DataSource = dt;
                    }
                    catch (Exception ex)
                    {
