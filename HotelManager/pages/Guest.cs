        ...
        private void search()
        {
            ...
            if (text != "")
            {
                try
                {
                    ...
                    {
                        dt = db.GetData("SELECT KHACHHANG.MAKH AS 'Guest ID', HOTEN AS 'Name', TUOI AS 'Age', DIACHI AS 'Address', SDT AS 'Phone Number',SOLANTHUE AS 'Times' FROM KHACHHANG WHERE HOTEN LIKE '%" + text + "%'");
					}
                    else if (activeFilterTab == "CHECKIN")
                    {
                        dt = db.GetData("SELECT KHACHHANG.MAKH AS 'Guest ID', HOTEN AS 'Name', TUOI AS 'Age', DIACHI AS 'Address', SDT AS 'Phone Number',SOLANTHUE AS 'Times' FROM KHACHHANG WHERE HOTEN LIKE '%" + text + "%' AND NGAYDI >= GETDATE()");
                    }
