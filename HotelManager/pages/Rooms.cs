        ...
        private void search()
        {
            ...
            if (text != "")
            {
                try
                {
                    ...
                    {
                        dt = db.GetData($"{selectAllCommand} WHERE SOPHONG LIKE '%{text}%'");
                    }
                    else if (activeFilterTab == "AVAILABLE")
                    {
                        dt = db.GetData($"{selectAllCommand} WHERE SOPHONG LIKE '%{text}%' AND TINHTRANG = 0");
                    }
                    else if (activeFilterTab == "BOOKED")
                    {
                        dt = db.GetData($"{selectAllCommand} WHERE SOPHONG LIKE '%{text}%' AND TINHTRANG = 1");
                    }
